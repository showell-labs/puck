...
A globally unique name for the plugin. When `render` is provided, `name` is required and for the plugin to appear within the plugin rail. The current open plugin is tracked in [`ui.plugin.current`](/docs/api-reference/data-model/app-state#uiplugincurrent)
